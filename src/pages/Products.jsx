...
const CATEGORIES = ["All", "Electronics", "Accessories", "Computers", "Wearables", "Cameras"];
const STATUSES = ["All", "Active", "Low Stock", "Inactive"];
const ITEMS_PER_PAGE = 8;

const statusConfig = {
  Active:      { bg: "bg-teal-500/20",    text: "text-teal-600",     dot: "bg-teal-500" },
  "Low Stock": { bg: "bg-[#d97706]/20",   text: "text-[#d97706]",    dot: "bg-[#d97706]" },
  Inactive:    { bg: "bg-[#dc2626]/20",   text: "text-[#dc2626]",    dot: "bg-[#dc2626]" },
};
...
export default function Products() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-6 text-[#0F3A53]">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#0F3A53] tracking-tight">Products</h1>
          <p className="text-gray-500 text-sm mt-0.5">Manage your product inventory</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total Products", value: totalProducts, color: "text-[#0F3A53]",  icon: "📦" },
          { label: "Active",         value: activeCount,   color: "text-teal-600",    icon: "✅" },
          { label: "Low Stock",      value: lowStockCount, color: "text-[#d97706]",   icon: "⚠️" },
          { label: "Inactive",       value: inactiveCount, color: "text-[#dc2626]",   icon: "🚫" },
        ].map((stat) => (
          <div
            key={stat.label}
      ...
      <div className="bg-white border border-[#0F3A53]/10 rounded-2xl px-5 py-4 mb-4 flex flex-wrap gap-4 items-end shadow-sm">
        ...
        <div className="ml-auto self-end">
          <span className="text-sm text-gray-500">
            <span className="text-[#0F3A53] font-semibold">{filtered.length}</span> products found
          </span>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#0F3A53]/10 bg-slate-50">
                ...
                <th className="text-right px-4 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Price</th>
                <th className="text-center px-4 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Stock</th>
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-3">
                          <img
                            ...
                            onError={(e) => {
                              e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(product.name)}&size=40&background=0F3A53&color=ffffff`;
                            }}
                          />
                          <span className="font-semibold text-[#0F3A53]">{product.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5 text-right">
                        <span className="font-bold text-[#0F3A53]">${product.price.toFixed(2)}</span>
                      </td>
      ...
      <div className="flex items-center justify-between mt-4 px-1">
        <span className="text-sm text-gray-500">
          ...
          of <span className="text-[#0F3A53] font-medium">{filtered.length}</span>
        </span>
